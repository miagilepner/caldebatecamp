...
function optimizer_home_query($query) {
	...
	if(!empty($optimizerdb) && empty($optimizer['converted'])) {
			if( $query->is_main_query() && is_front_page() ) {
				...
				if(!empty($optimizer['n_pages_id'])){
					$pageids = $optimizer['n_pages_id'];
					set_query_var( 'post__in', $pageids );
				}
...
*/
	...
	if(!empty($optimizer['custom_sidebar'])){
		...
		if(!empty($optimizerdb) && empty($optimizer['converted'])) {
				...
				if(!array_filter($optimizer['custom_sidebar'])) {  $customsidebars = '';  }else{  $customsidebars = $optimizer['custom_sidebar']; }
			  	$custom_sidebar = $optimizer['custom_sidebar']; 
			...
			foreach((array)$custom_sidebar as $key => $value){
				register_sidebar(array(
					'name'          => $value,
					'id'            => 'optimizer_'.str_replace( ' ','-',strtolower(trim($value))),
...
function optimizer_send_message($to) {
        ...
        if (isset($_POST['contact_message'])) {
			...
			$headers = "From: ".trim($_POST['contact_name'])." <".trim($_POST['contact_email']).">\r\nReply-To:".trim($_POST['contact_email']);
            $subject = get_bloginfo('name')." | ". $_POST['contact_subject'];
            
            ob_start();
            
            	echo wpautop($_POST['contact_message']);
