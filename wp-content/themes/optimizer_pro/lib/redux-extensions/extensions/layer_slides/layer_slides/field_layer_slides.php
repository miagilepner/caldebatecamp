...
// Don't duplicate me!
if( !class_exists( 'ReduxFramework_layer_slides' ) ) {

    /**
     * Main ReduxFramework_layer_slides class
     *
     * @since       1.0.0
     */
    class ReduxFramework_layer_slides{
        ...
        public function render() {
            ...
            $defaults = array(
                ...
                'content_title' => __ ( 'Slide', 'optimizer' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );

            echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New Slide', 'optimizer' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                foreach ( $slides as $slide ) {
                    ...
                    $defaults = array(
                        ...
                        'image' => '',
                        'thumb' => '',
                        'button_text' => '',/*OPTIMIZER CUSTOM MARKUP*/
                        'style' => '',/*OPTIMIZER CUSTOM MARKUP*/
                        'attachment_id' => '',
                        'height' => '',
                        'width' => '',
                        'select' => array(),
                    ...
                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __ ( 'Upload', 'optimizer' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __ ( 'Remove', 'optimizer' ) . '</span>';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'optimizer' );
                    ...
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'optimizer' );
					...
					/*------------------------------------OPTIMIZER CUSTOM MARKUP STARTS-------------------------------*/
					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'button_text' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'button_text' ] ) : __ ( 'Button Text', 'optimizer' );
                    echo '<li><input type="text" id="' . $this->field[ 'id' ] . '-btntxt_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][button_text]' . $this->field['name_suffix'] .'" value="' . esc_attr ( $slide[ 'button_text' ] ) . '" class="full-text" placeholder="' . $placeholder . '" /></li>';

					/*------------------------------------OPTIMIZER CUSTOM MARKUP ENDS-------------------------------*/
					
					
										
 					$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'optimizer' );
					...
					/*------------------------------------OPTIMIZER CUSTOM MARKUP STARTS-------------------------------*/
                    echo '<li><p><label for="'. $this->field['id'] .  '-style_' . $x . '">';
                    echo '<input type="checkbox" class="checkbox-slide-style" id="' . $this->field['id'] . '-style_' . $x . '" value="1" ' . checked(  $slide['style'], '1', false ) . ' name="' . $this->field['name'] . '[' . $x . '][style]" />';
                    echo ' '.__('Hollow Button Style', 'optimizer'). '</label></p></li>';
					/*------------------------------------OPTIMIZER CUSTOM MARKUP ENDS-------------------------------*/
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __ ( 'Delete', 'optimizer' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'optimizer' ) . '</span>';
				...
				/*------------------------------------OPTIMIZER CUSTOM MARKUP STARTS-------------------------------*/
				$placeholder = ( isset ( $this->field[ 'placeholder' ][ 'button_text' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'button_text' ] ) : __ ( 'Button Text', 'optimizer' );
                echo '<li><input type="text" id="' . $this->field[ 'id' ] . '-btntxt_' . $x . '" name="' . $this->field[ 'name' ] . '[' . $x . '][button_text]' . $this->field['name_suffix'] .'" value="" class="full-text" placeholder="' . $placeholder . '" /></li>';
				/*------------------------------------OPTIMIZER CUSTOM MARKUP ENDS-------------------------------*/
				...
				/*------------------------------------OPTIMIZER CUSTOM MARKUP STARTS-------------------------------*/
				echo '<li><label for="'. $this->field['id'] .  '-style_' . $x . '">';
                echo '<input type="checkbox" class="checkbox-slide-style" id="' . $this->field['id'] . '-style_' . $x . '" value="" ' . checked(  '', '1', false ) . ' name="' . $this->args['opt_name'] . '[' . $this->field['id'] . '][' . $x . '][style]" />';
                echo ' '.__('Hollow Button Style', 'optimizer'). '</label></li>';
				/*------------------------------------OPTIMIZER CUSTOM MARKUP ENDS-------------------------------*/
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add Slide', 'optimizer' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        ...
        public function enqueue() {
            ...
            //$extension = ReduxFramework_extension_layer_slides::getInstance();
            ...
            wp_enqueue_script (
                'extension-layer_slides-js', 
                get_template_directory_uri() . '/lib/redux-extensions/extensions/layer_slides/layer_slides/field_layer_slides' . Redux_Functions::isMin () . '.js', 
            ...
            wp_enqueue_style (
                'extension-layer_slides-css', 
                get_template_directory_uri() . '/lib/redux-extensions/extensions/layer_slides/layer_slides/field_layer_slides.css', 
                time (), 
                true
            );        
        }
