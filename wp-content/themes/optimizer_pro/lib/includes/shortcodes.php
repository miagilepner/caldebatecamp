...
function layersc_scs( $atts, $content = null ) {
		...
		return '<div class="lts_scs '.$class.'">'.do_shortcode($content).'</div>';
}
...
function layersc_info( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '',), $atts));
		...
		return '<div class="lts_info '.$class.'">'.do_shortcode($content).'</div>';
}
...
function layersc_wng( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '',), $atts));
		...
		return '<div class="lts_wng '.$class.'">'.do_shortcode($content).'</div>';
}
...
function layersc_err( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '',), $atts));
		...
		return '<div class="lts_err '.$class.'">'.do_shortcode($content).'</div>';
}
...
function layersc_quote( $atts, $content = null ) {
	...
	if($author !== ''){
		...
		return '<div class="lts_quote_wrap '.$class.'"><div class="lts_quote">'.do_shortcode($content).'</div><div class="lts_quote_author"><a target="_blank" href="'.$source.'">'.$author.'</a></div></div>';
}
...
function layersc_divider( $atts, $content = null ) {
		...
		return '<div class="ast_divide '.$class.'" style="clear:both;border-bottom: '.$height.' '.$style.' '.$color.'; width:100%; height:2px; margin:15px 0;"></div>';
}
...
function layersc_tdivider( $atts, $content = null ) {
		extract(shortcode_atts(array(
		'style' => 'fa-stop', //fa-stop, underline, fa-star, fa-times, fa-bolt, fa-asterisk, fa-chevron-down, fa-heart, fa-plus, fa-bookmark, fa-circle-o, fa-th-large, fa-minus, fa-cog, fa-reorder
		'color' => '#333',
		'class'=> '',
	), $atts));
	if($style == 'underline'){ $underline= 'title_underline';}else{$underline='';}
	
		return '<div class="optimizer_divider '.$class .$underline.'">
          <span class="div_left" style="background:'.$color.'"></span>
          <span class="div_middle" style="color:'.$color.'"><i class="fa '.$style.'"></i></span>
          <span class="div_right" style="background:'.$color.'"></span>
      </div>';
}
...
function layersc_slider( $atts, $content = null ) {
	...
	if($autoplay == 'true'){
		$autoplay = 'manualAdvance:false';
	}else{
		$autoplay = '';
	}
	
	$content= preg_replace('<<br />>', '', $content);
		return '<div class="ast_slide_wrap '.$class.'"><div class="ast_slider">'.$content.'</div></div><script>jQuery(function(){jQuery(".ast_slider").nivoSlider({ effect :"'.$effect.'", pauseTime: '.$pausetime.', directionNav: '.$navigation.', '.$autoplay.'});   });</script>';
}
...
<div class="fb-like" data-href="'.urlencode(get_permalink()).'" data-layout="'.$layout.'" data-action="'.$action.'" data-show-faces="false" data-share="'.$share.'"></div>
				';
...
function layersc_tweet( $atts, $content = null) {
   ...
   return '<a href="https://twitter.com/share" class="twitter-share-button" '.$via.' '.$count.' '.$size.' '.$count_type.'>Tweet</a>
...
function layersc_pinit( $atts, $content = null) {
   ...
   return '<a href="//pinterest.com/pin/create/button/" data-pin-do="buttonBookmark" '.$size.' '.$style.'><img src="//assets.pinterest.com/images/pidgets/pin_it_button.png" /></a><script type="text/javascript" src="//assets.pinterest.com/js/pinit.js"></script>';
}
...
function layersc_gplus( $atts, $content = null) {
   ...
   return '<div style="'.$width.' display: inline-block;" class="gplus_wrap gplus_'.$style.'"><div class="g-plusone" '.$size.' '.$annotation.'></div></div><script type="text/javascript">
  (function() {
...
function layersc_list_func($atts, $content) {
	...
	$return_html = '<div class="lts_list '.$style.' '.$class.'" data-list-color="'.$bullet_color.'">'.$content.'</div>';
...
if( !class_exists( 'Jetpack' )){
		function layersc_youtube_func($atts, $content) {
			...
			$return_html = '<div style="clear:both;"></div><div class="ast_vid '.$class.'"><div class="responsive-container"><'.$node.' class="vid_iframe" style=" width: '.$width.'; height: '.$height.';" src="'.$content.''.$autoplay.'" allowfullscreen></'.$node.'></div></div>';
...
if( !class_exists( 'Jetpack' )){
		function layersc_vimeo_func($atts, $content) {
			...
			$return_html = '<div style="clear:both;"></div><div class="ast_vid '.$class.'"><div class="responsive-container"><'.$node.' class="vid_iframe" style=" width: '.$width.'; height: '.$height.';" src="'.$content.'?title=1&amp;byline=0&amp;portrait=0&amp;color=00adef&amp;autoplay='.$autoplay.'"></'.$node.'></div></div>';
...
function layersc_flickr_func( $atts, $content = null) {
	...
	$return_html = '<div id="flickr_badge_wrapper" class="clearfix flckr_'.$photo_size.' '.$class.'">
...
function layersc_col2_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		return '<div class="col2 shortcol'.$class.'" data-width="'.$width.'">'.do_shortcode($content).'</div>';
}
...
function layersc_col3_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		return '<div class="col3 shortcol '.$class.'" data-width="'.$width.'">'.do_shortcode($content).'</div>';
}
...
function layersc_col4_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		return '<div class="col4 shortcol '.$class.'" data-width="'.$width.'">'.do_shortcode($content).'</div>';
}
...
function layersc_col5_func( $atts, $content = null ) {
	extract(shortcode_atts(array('class' => '','width' => '',), $atts));
		...
		return '<div class="col5 shortcol '.$class.'" data-width="'.$width.'">'.do_shortcode($content).'</div>';
}
...
function layersc_tabs($atts, $content = null) {
	...
	$html='<div class="tabs-container lts_tabs tabs_'.$style.' '.$class.'" data-active-color="'.$active_color.'"><ul class="tabs ">';
...
function layersc_tab($atts, $content = null) {
	return '<div class="lts_tab_child">'.do_shortcode($content).'</div>';
}
...
function layersc_toggle($atts, $content = null) {
	...
	$html='<div class="lts_toggle '.$class.'"><div class="trigger_wrap">';
	foreach($titlearr as $title){
...
function layersc_button_func($atts, $content = null) {
	...
	$return_html = '<a class="lts_button lts_button_'.$size.' '.$rounded.' '.$style.' '.$class.'" '.$open_new_window.' style="background:'.$background_color.';color:'.$text_color.'!important;border-color:'.$background_color.';" href="'.$url.'">'.$icon .$text.'</a>';
...
function layersc_callaction_func($atts, $content = null) {
	...
	$return_html = '<div style="background:'.$background_color.';color:'.$text_color.'!important;" class="ast_shrt_action'.$rounded.' '.$class.'"><div class="act_left">'.$content.'</div><div class="act_right"><a class="'.$rounded.'" style="background:'.$button_background_color.';color:'.$button_text_color.'!important;" href="'.$button_url.'">'.$button_text.'</a></div></div><div style="clear:both"></div>';
...
function layersc_table_func( $atts ) {
    ...
    $total = count($cols);
	$output = "";
    $output .= '<table class="ast_table '.$class.'"><tr class="th">';
    foreach($cols as $col):
        $output .= '<td>'.$col.'</td>';
    endforeach;
    $output .= '</tr><tr>';
...
if (has_post_thumbnail() ) {
    ...
    $result = '<div class="'.$class.'" style="width:100%;"><img src="'.$image_url.'" class="'.$align.'" /><div style="clear:both"></div></div>';
    return $result;
}
return;
}
...
function layersc_tooltip( $atts, $content = null ) {
	extract(shortcode_atts(array('tipcontent' => '',  'class' => '',), $atts));
		...
		return '<span class="tooltip lts_tooltip '.$class.'" title="'.$tipcontent.'">'.do_shortcode($content).'</span>';
}
...
function layersc_dropcap( $atts, $content = null ) {
	extract(shortcode_atts(array('color' => '#fff' , 'background' => '#2dcb73', 'class' => '',), $atts));
		...
		return '<span style="color:'.$color.';background:'.$background.';" class="lts_dropcap '.$class.'">'.do_shortcode($content).'</span>';
}
...
function layersc_progress( $atts, $content = null ) {
	...
	if($stripes == "true"){
		...
		return '<div class="lts_progress '.$rounded. $stripes.' '.$class.'"><span>'.$percent.'%</span><div class="lts_progress_wrap"><div title="'.$percent.'%" class="lts_progress_inner tooltip" style="width:'.$percent.'%;color:'.$color.';background-color:'.$background.';"></div></div></div>';
}
...
function layersc_section( $atts, $content = null ) {
	...
	if(!empty($background_image)){
	  ...
	  return '<div style="background-color:'.$background_color.'; color:'.$text_color.';'.$background_image.'" class="lts_section '.$class.'"><div class="lts_section_body">'.do_shortcode($content).'</div></div>';
}
...
function layersc_block( $atts, $content = null ) {
	...
	if($rounded =='true'){
		...
		return '<div class="lts_block '.$shadow.' '.$rounded.' '.$class.'" style="background:'.$background.';color:'.$text_color.';">'.do_shortcode($content).'</div>';
}
...
function layersc_blocks( $atts, $content = null ) {
	...
	'class'=> '',
		...
		return '<div class="lts_blocks lts_blocks_'.$layout.' '.$class.'">'.do_shortcode($content).'</div>';
}
...
function layersc_panel( $atts, $content = null ) {
	extract(shortcode_atts(array('title' => '', 'text_color' => '#ffffff','background_color' => '#428bca','class'=> '',), $atts));
		...
		return '<div style="border-color:'.$background_color.'" class="lts_panel '.$class.'"><h3 style="background-color:'.$background_color.';color:'.$text_color.'">'.$title.'</h3><div class="lts_panel_body">'.do_shortcode($content).'<div style="clear:both"></div></div></div>';
}
...
function layersc_author( $atts, $content = null ) {
	...
	if($website !==''){ $website ='<a href="'.esc_url($website).'"><i class="fa fa-globe"></i></a>';  }else{  $website ='';  }
		...
		return '
		<div class="lts_author '.$class.'" style="background:'.$background.'">
			'.$image.'
...
function layersc_carousel( $atts, $content = null ) {
	...
	'class' => '',
		...
		return '
		<div class="looper slide lts_looper lts_'.$style.' '.$class.'" data-looper="go" data-buttoncolor="'.$button_color.'" style="background:'.$background.';color:'.$text_color.'">
...
function layersc_searchform( $atts, $content = null ) {
	extract(shortcode_atts(array('label' => 'Search', 'text_color' => '#666666','background_color' => '#eeeeee','class'=> '',), $atts));
	  ...
	  return '<form role="search" method="get" class="lts_search-form '.$class.'" action="'.home_url( '/' ).'">
...
function layersc_pricing( $atts, $content = null ) {
	...
	'class' => '',
		...
		return '
		<div class="lts_pricing '.$class.'" style="color:'.$text_color.'" data-button-bg="'.$button_bg_color.'" data-button-txt="'.$button_text_color.'" data-price-bg="'.$background.'" data-price-txt="'.$text_color.'">
		'.do_shortcode($content).'
		</div>	
			';
}
...
function layersc_lightbox_func($atts, $content) {
	...
	$return_html = '<span style="'.$button_text_color. $button_text_bg .$button_font_size.'" class="lts_lightbox_bttn lt_animate '.$rounded.' '.$class.'">'.$button_text.'</span><div style="display:none" class="lts_lightbox_content">'.do_shortcode($content).'</div>';
...
function layersc_map( $atts, $content = null ) {
	...
	return '<div class="lts_map_wrap '.$class.'" id="map_1" style="height:'.$height.'"><div data-map-lat="'.$latitude.'" data-map-long="'.$longitude.'" data-map-text="'.$text.'" class="lts_map"></div></div>';
}
...
function layersc_icon( $atts, $content = null ) {
	...
	return '<i style="color:'.$color.';font-size:'.$size.'" class="fa '.$type.' '.$class.'"></i>';
}
...
function layersc_headline( $atts, $content = null ) {
	...
	return '<'.$size.' class="lts_headline_parent lts_headline_'.$type.' '.$class.'"><span style="color:'.$color.';" class="lts_headline headline_'.$type.'">'.do_shortcode($content).'</span></'.$size.'>';
}
...
function optimizer_display_posts_shortcode( $atts ) {
	...
	while ( $listing->have_posts() ): $listing->the_post(); global $post;
		...
		if ( $image_size && has_post_thumbnail() )  
			...
			$image = '<div class="img_wrap"><a href="' . get_permalink() . '"><i class="optimizer_plus">+</i></a><a class="image" href="' . get_permalink() . '"><img src="' . optimizer_placeholder_image(). '" /></a></div>';
		}
		...
		if( $layout == 'layout4' || $layout == 'layout5' )
			$cats = apply_filters( 'display_posts_shortcode_author', ' <span class="lt_cats"><i class="fa-th-list"></i> ' . get_the_category_list() . '</span>' );
			
		if ( $include_excerpt ) 
			if($layout == 'layout2' || $layout == 'layout3'){
			$excerpt = '<span class="excerpt">' . wp_trim_words(get_the_excerpt(), 18) . '</span>';
			}
			if($layout == 'layout4'){
			$excerpt = '<span class="excerpt">' . get_the_excerpt(). '</span><div class="blog_mo"><a href="' . get_permalink() . '">'.__('+ Read More', 'optimizer').'</a></div>';
			}
