...
class optimizer_front_About extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if(is_customize_preview()){
			...
			echo '<style>#'.$id.'{ ' . $content_bg . '' . $content_bgimg . '}#'.$id.' .about_header, #'.$id.' .about_pre, #'.$id.' span.div_middle{color: ' . $title_color . '}#'.$id.' span.div_left, #'.$id.' span.div_right{background-color: ' . $title_color . '}#'.$id.' .about_content{' . $content_color . '}#'.$id.' .about_inner {'.$aboutwidth.'}</style>';
		}
...
<?php
		...
		if(!is_customize_preview()){
			...
			foreach ( $settings as $instance_id => $instance ) {
				...
				$widget_style = '#'.$id.'{ ' . $content_bg . '' . $content_bgimg . '}#'.$id.' .about_header, #'.$id.' .about_pre, #'.$id.' span.div_middle{color: ' . $title_color . '}#'.$id.' span.div_left, #'.$id.' span.div_right{background-color: ' . $title_color . '}#'.$id.' .about_content{' . $content_color . '}#'.$id.' .about_inner{' . $aboutwidth . '}';
