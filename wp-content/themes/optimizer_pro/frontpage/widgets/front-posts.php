...
class optimizer_front_Posts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		<div class="lay'.$layout.' optimposts" data-post-layout="'.$layout.'" data-post-type="'.$type.'" data-post-count="'.$count.'" data-post-category="'.$blogcats.'" data-post-pages="'.$widgetpages.'" data-post-previewbtn="'.$previewbtn.'" data-post-linkbtn="'.$linkbtn.'" data-post-navigation="'.$navigation.'">
		<div class="center">';
			...
			//Call the Posts
			optimizer_posts($layout, $type, $count, $category, $pages, $previewbtn , $linkbtn, $navigation);
		...
		if(is_customize_preview()){
			...
			echo '<style>#'.$id.' .lay'.$layout.' .hentry, #'.$id.' .lay'.$layout.' .lay2_wrap .type-product{ '.$postbgcolor.' }#'.$id.' .lay'.$layout.'{ '.$secbgcolor.' }#'.$id.' .lay'.$layout.' .home_title, #'.$id.' .lay'.$layout.' .home_subtitle, #'.$id.' span.div_middle{color:'.$titlecolor.' }#'.$id.' span.div_left, #'.$id.' span.div_right{background-color:' . $titlecolor . '}</style>';
...
<?php
	...
	function optimizer_masonry_run(){
		...
		if($instance['layout'] == 3){
			echo '<script type="text/javascript">
			//Layout3 Masonry
			
			var container'.$idbase.' = document.querySelector("#'.$this->id.' .lay3_wrap");
			var msnry;
			if(container'.$idbase.'){
    ...
    function front_posts_enqueue_css() {
		...
		foreach ( $settings as $instance_id => $instance ) {
			...
			$widget_style = '#'.$id.' .lay'.$layout.' .hentry, #'.$id.' .lay'.$layout.' .lay2_wrap .type-product{ '.$postbgcolor.' }#'.$id.' .lay'.$layout.'{ '.$secbgcolor.' }#'.$id.' .lay'.$layout.' .home_title, #'.$id.' .lay'.$layout.' .home_subtitle, #'.$id.' span.div_middle{color:'.$titlecolor.' }#'.$id.' span.div_left, #'.$id.' span.div_right{background-color:' . $titlecolor . '}';
